...
import { generate as PrismaTrpcShieldGenerator } from "./prisma-trpc-shield-generator/prisma-generator";
import { generate as PrismaZodGenerator } from "./prisma-zod-generator/prisma-generator";
...
export async function generate(options: GeneratorOptions) {
  ...
  let shieldOutputPath: string | undefined;
  ...
  if (config.withShield) {
    generateShieldImport(createRouter, shieldOutputPath as string);
  }
  ...
  prismaClientDmmf.mappings.modelOperations.forEach((modelOperation) => {
    ...
    for (const [opType, opNameWithModel] of Object.entries(operations)) {
      generateProcedure(
        modelRouter,
        opNameWithModel as string,
        getInputTypeByOpName(opType, model) as string,
        model,
        opType
      );
    }
